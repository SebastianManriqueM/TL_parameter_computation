...
function get_primitive_z_matrix( 
    ...
    for i in geometry.combinations
        z_primitive[i[1] , i[2]] = R_CONST_OHM_MILE*freq + ( L_CONST_OHM_MILE*freq*( log(1/geometry.distances[j]) + L_INDTERM_OHM_MILE + 0.5*log(rho/freq) ) )*im
    ...
    for i = 1:n_cond
        ...
        if i <= 3  #Circuit 1
            z_primitive[i , i] = conductor.Rac_tnom + R_CONST_OHM_MILE*freq + ( L_CONST_OHM_MILE*freq*( log(1/conductor.gmr)   + L_INDTERM_OHM_MILE + 0.5*log(rho/freq) ) )*im
        elseif  basicdata.n_circuits == 2 && i <= basicdata.n_circuits * 3  #Circuit 2
            z_primitive[i , i] = conductor.Rac_tnom + R_CONST_OHM_MILE*freq + ( L_CONST_OHM_MILE*freq*( log(1/conductor.gmr)   + L_INDTERM_OHM_MILE + 0.5*log(rho/freq) ) )*im
        else #Ground wire
            z_primitive[i , i] = ground_wire.Rdc_20 + R_CONST_OHM_MILE*freq + ( L_CONST_OHM_MILE*freq*( log(1/ground_wire.gmr) + L_INDTERM_OHM_MILE + 0.5*log(rho/freq) ) )*im
        end
